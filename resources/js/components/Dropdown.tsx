...
import { ButtonHTMLAttributes, createContext, Dispatch, Fragment, PropsWithChildren, SetStateAction, useContext, useState } from 'react';
...
const Content = ({
    ...
    return (
        <>
            ...
            >
                <div className={`absolute z-50 ${alignmentClasses} ${widthClasses}`} onClick={() => setOpen(false)}>
                    <div
                        className={
                            `ring-opacity-5 dark:ring-opacity-5 rounded-lg bg-background !p-2 shadow-md ring-1 ring-black dark:ring-white ` +
                            contentClasses
                        }
                    >
                        {children}
...
const DropdownButton = ({ className = '', children, ...props }: PropsWithChildren<ButtonHTMLAttributes<HTMLButtonElement>>) => {
    return (
        <button
            {...props}
            className={
